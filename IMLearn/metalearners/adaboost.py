...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        n_samples = X.shape[0]
        self.D_ = 1.0/n_samples * np.ones(n_samples)
        self.models_ = [None] * self.iterations_
        self.weights_ = np.zeros(self.iterations_)

        for t in range(self.iterations_):
            self.models_[t] = self.wl_()
            self.models_[t]._fit(X, self.D_*y)
            y_hat = self.models_[t]._predict(X)
            epsilon = np.sum(self.D_ * (np.abs(y_hat - y) / 2))
            self.weights_[t] = 0.5 * np.log(1.0/epsilon - 1)
            self.D_ *= (np.exp((-1) * self.weights_[t] * y * y_hat))
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        for t in range(T):
            y_pred += (self.weights_[t] * self.models_[t]._predict(X))
